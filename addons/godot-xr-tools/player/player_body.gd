...
## Distance from the ground to be considered "on" the ground
const ON_GROUND_DISTANCE := 0.1

## Distance from the ground to be considered "near" the ground
...
## Whether to automatically calibrate the player body on the next frame
...
## Distance between between the camera and the top of the head
...
## Eyes forward offset from center of body in [member player_radius] units
@export_range(0.0, 1.0) var eye_forward_offset := 0.5

## How much the hand controllers affect body orientation
...
## [b]Fade[/b]: fades view to black.[br][br]
## [b]Disabled[/b]: do nothing.
...
## When ground control is allowed
...
## Up direction, according to gravity
var up_gravity := Vector3.UP

## Up direction, according to player body
...
# Current height of the player when overriden
var _player_height_override_current := 0.0

# Target height of the player when overriden
...
func _physics_process(delta: float) -> void:
	...
	var delta_transform := global_transform * current_transform.inverse()
...
func move_player(p_velocity: Vector3) -> Vector3:
	...
	var transform_before_movement := global_transform
...
func request_jump(skip_jump_velocity: bool = false) -> void:
	...
	if absf(ground_relative.dot(ground_vector)) > 0.01:
		return
...
func slew_up(up: Vector3, slew: float) -> void:
	...
	var ref_pos_local := ref_pos_global * current_origin
...
func _update_body_under_camera(delta: float) -> void:
	...
	if _player_height_override_enabled:
		...
		if _player_height_override_lerp <= 0.0:
			...
			_player_height_override_current = minf(
