...
@onready var laser_mesh: MeshInstance3D = $Laser

@onready var target_mesh: MeshInstance3D = $Target

@onready var suppress_area: Area3D = $SuppressArea

@onready var suppress_collider: CollisionShape3D = $SuppressArea/CollisionShape3D

@onready var visualise_mesh: MeshInstance3D = $Visualise
...
func _ready() -> void:
	...
	material = visualise_mesh.get_surface_override_material(0)
...
func _process(delta: float) -> void:
	...
	if (
			enabled 
			and not suppress_area.has_overlapping_bodies()
			and not suppress_area.has_overlapping_areas()
...
func set_show_target(p_show_target: bool) -> void:
	...
	if is_inside_tree():
		target_mesh.visible = enabled and show_target and last_target
...
func _update_y_offset() -> void:
	laser_mesh.position.y = y_offset * _world_scale
...
func _update_target_radius() -> void:
	target_mesh.mesh.radius = target_radius
	target_mesh.mesh.height = target_radius * 2


## Pointer target_material update handler
func _update_target_material() -> void:
	target_mesh.set_surface_override_material(0, target_material)
...
func _update_suppress_radius() -> void:
	suppress_collider.shape.radius = suppress_radius


## Pointer suppress_mask update handler
func _update_suppress_mask() -> void:
	suppress_area.collision_mask = suppress_mask
...
func _update_laser_active_material(hit : bool) -> void:
	if hit and laser_hit_material:
		laser_mesh.set_surface_override_material(0, laser_hit_material)
	else:
...
func _visible_hit(at : Vector3) -> void:
	...
	if show_target:
		target_mesh.global_transform.origin = at
	...
	if show_laser != LaserShow.HIDE:
		...
		if laser_length == LaserLength.COLLIDE:
			...
			laser_mesh.mesh.size.z = collide_len
			laser_mesh.position.z = collide_len * -0.5
		else:
			laser_mesh.mesh.size.z = distance
			laser_mesh.position.z = distance * -0.5
...
func _visible_move(at : Vector3) -> void:
	...
	visualise_mesh.global_transform.origin = at
...
func _visible_miss() -> void:
	...
	laser_mesh.visible = show_laser == LaserShow.SHOW
...
func _set_time_held(p_time_held):
	...
	if material:
		visualise_mesh.visible = time_held > 0.0
...
func _update_size():
	if material: # Note, material won't be set until after we setup our scene
		var mesh : QuadMesh = visualise_mesh.mesh
		if mesh.size != size:
			mesh.size = size

			# updating the size will unset our material, so reset it
			visualise_mesh.set_surface_override_material(0, material)
