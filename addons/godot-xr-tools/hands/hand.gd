...
## grip and trigger animations based on controller input.[br][br]
##
...
## Emitted when the hand scale changes
signal hand_scale_changed(scale: float)
...
## Input action for the Grip action in the OpenXR Action Map.
@export var grip_action := "grip"

## Input action for the Trigger action in the OpenXR Action Map.
...
var _pose_overrides: Array[PoseOverride]
...
## Searches from the specified node for the left controller [XRToolsHand],
## assuming the node is a sibling of the [XROrigin3D].
...
## Searches from the specified node for the right controller [XRToolsHand],
## assuming the node is a sibling of the [XROrigin3D].
...
# Checks for world-scale changes and scales itself causing the hand mesh and
# skeleton to scale appropriately. It then reads the grip and trigger action
# values to animate the hand.
func _physics_process(_delta: float) -> void:
	...
	if XRServer.world_scale != _last_world_scale:
		...
		hand_scale_changed.emit(_last_world_scale)

	# Animate the hand mesh with the controller inputs
	if _controller:
		var grip := _controller.get_float(grip_action)
		var trigger := _controller.get_float(trigger_action)
	...
	if _target is XRToolsGrabPointHand:
		...
		if hand_offset_mode != 4:
			target_transform = target_transform * XRTools.get_palm_offset(
					hand_offset_mode,
					_controller,
			).inverse()
	else:
...
## Adds a target override. The collision hand will attempt to move to the
## highest priority target, or the [XRController3D] if no override is specified.
...
## Forces the grip and trigger values (primarily for preview)
...
## Sets the default open-hand pose
...
func _insert_pose_override(
	...
	for pos: int in _pose_overrides.size():
...
# Inserts a target override into the overrides list by priority order
func _insert_target_override(target: Node3D, priority: int) -> void:
	...
	for pos: int in _target_overrides.size():
...
# Removes a target from the overrides list
...
func _update_pose() -> void:
	...
	if open_pose:
		...
		if open_name == "":
			...
			_animation_player.get_animation_library("").add_animation(
					open_name,
					open_pose,
			)
...
# Updates the target for hand movement.
